...
export function TVShowForm({ onSubmit, initialData, isEditing = false }: TVShowFormProps) {
  const [formData, setFormData] = useState<TVShowFormData>(initialData || {
    title: '',
    director: '',
    budget: 0,
    location: '',
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof TVShowFormData, string>>>({});
  const [touched, setTouched] = useState<Partial<Record<keyof TVShowFormData, boolean>>>({});

  const validateField = (field: keyof TVShowFormData, value: any) => {
    try {
      const schema = tvShowSchema.shape[field];
      schema.parse(value);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        const newErrors: Partial<Record<keyof TVShowFormData, string>> = {};
        error.errors.forEach(err => {
          if (err.path[0]) {
            newErrors[err.path[0] as keyof TVShowFormData] = err.message;
          }
        });
        setErrors(newErrors);
        // Mark all fields as touched when form is submitted with errors
        const allTouched: Partial<Record<keyof TVShowFormData, boolean>> = {};
        Object.keys(formData).forEach(key => {
          allTouched[key as keyof TVShowFormData] = true;
        });
  ...
  const handleBlur = (field: keyof TVShowFormData) => {
